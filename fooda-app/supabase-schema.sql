-- Fooda — baseline schema (idempotent: safe to re-run)
...
CREATE POLICY "Vendors can view their own profile"   ON vendors FOR SELECT USING (owner_id = auth.uid());
CREATE POLICY "Vendors can update their own profile" ON vendors FOR UPDATE USING (owner_id = auth.uid());
CREATE POLICY "Vendors can insert their own profile" ON vendors FOR INSERT WITH CHECK (owner_id = auth.uid());
...
CREATE POLICY "Everyone can view vendor menu categories" ON menu_categories
  FOR SELECT USING (EXISTS (SELECT 1 FROM vendors WHERE id = vendor_id AND is_active = true));
...
CREATE POLICY "Everyone can view available menu items" ON menu_items
  FOR SELECT USING (is_available = true AND EXISTS (SELECT 1 FROM vendors WHERE id = vendor_id AND is_active = true));

CREATE POLICY "Vendors can manage their own menu items" ON menu_items
  FOR ALL USING (EXISTS (SELECT 1 FROM vendors WHERE id = vendor_id AND owner_id = auth.uid()));
...
CREATE POLICY "Users can view their own addresses"   ON user_addresses FOR SELECT USING (user_id = auth.uid());
CREATE POLICY "Users can insert their own addresses" ON user_addresses FOR INSERT WITH CHECK (user_id = auth.uid());
CREATE POLICY "Users can update their own addresses" ON user_addresses FOR UPDATE USING (user_id = auth.uid());
CREATE POLICY "Users can delete their own addresses" ON user_addresses FOR DELETE USING (user_id = auth.uid());
...
CREATE POLICY "Vendors can view their own orders"         ON orders FOR SELECT USING (vendor_id IN (SELECT id FROM vendors WHERE owner_id = auth.uid()));
CREATE POLICY "Delivery persons can view assigned orders" ON orders FOR SELECT USING (delivery_person_id = auth.uid());
CREATE POLICY "Customers can insert their own orders"     ON orders FOR INSERT WITH CHECK (customer_id = auth.uid());
...
-- delivery_persons
DROP POLICY IF EXISTS "Delivery persons can view their own profile"   ON delivery_persons;
DROP POLICY IF EXISTS "Delivery persons can update their own profile" ON delivery_persons;

CREATE POLICY "Delivery persons can view their own profile"   ON delivery_persons FOR SELECT USING (user_id = auth.uid());
CREATE POLICY "Delivery persons can update their own profile" ON delivery_persons FOR UPDATE USING (user_id = auth.uid());

-- reviews
DROP POLICY IF EXISTS "Everyone can view verified reviews"              ON reviews;
DROP POLICY IF EXISTS "Customers can insert reviews for their orders"   ON reviews;

CREATE POLICY "Everyone can view verified reviews" ON reviews FOR SELECT USING (is_verified = true);
CREATE POLICY "Customers can insert reviews for their orders" ON reviews
  FOR INSERT WITH CHECK (
    customer_id = auth.uid()
    AND EXISTS (SELECT 1 FROM orders WHERE id = order_id AND customer_id = auth.uid())
  );
